    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-semibold mb-4 text-gray-900">Novo Horário</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, DiaDaSemana: Number(e.target.value) })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm({ ...form, HorarioDeInicio: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-gray-900"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm({ ...form, HorarioDeEncerramento: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-gray-900"
                />
              </div>
              <div className="flex items-end">
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, Intervalo: e.target.checked })}
                    className="rounded"
                  />
                  Intervalo
                </label>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">Cancelar</button>
            </div>
          </form>
        </div>
      )}
          ...
          <div key={day} className="bg-white rounded-lg shadow p-4">
            ...
            {blocks.length === 0 ? (
              ...
              <div className="space-y-1">
                {blocks.map((b) => (
                  <div key={b.HorarioId} className="flex items-center justify-between text-sm">
                    <span className={b.Intervalo ? "text-orange-600" : "text-gray-700"}>
                      {b.HorarioDeInicio} - {b.HorarioDeEncerramento}
                      {b.Intervalo && " (Intervalo)"}
                    </span>
                    <button onClick={() => handleDelete(b.HorarioId)} className="text-red-500 hover:underline text-xs">Remover</button>
                  </div>
                ))}
              </div>
            )}
